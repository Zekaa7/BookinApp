...
async function SearchPage({ searchParams }: Props) {
  ...
  const numberOfNights: number = differenceInTime / (1000 * 3600 * 24);
                    ...
                    <div className="text-right">
                      <p className="text-xs">
                        {`${Number(numberOfNights)} ${
                          Number(numberOfNights) < 2 ? "night" : "nights"
                        }, ${Number(searchParams.group_adults)} ${
                          Number(searchParams.group_adults) < 2
                            ? "adult"
                            : "adults"
                        }${
                          Number(searchParams.group_children) > 0
                            ? `, ${Number(searchParams.group_children)} ${
                                Number(searchParams.group_children) < 2
                                  ? "child"
                                  : "children"
                              }`
                            : ""
